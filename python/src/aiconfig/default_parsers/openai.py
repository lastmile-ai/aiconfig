        ...
        while i < len(conversation_data["messages"]):
            ...
            if role == "user" or role == "function":
                ...
                if assistant_response is not None:
                    ...
                    if hasattr(assistant_response, "role") and assistant_response.role is not None:
                        metadata["role"] = assistant_response.role
                    assistant_output = [
                        ExecuteResult(
    ...
    async def run_inference(
        ...
        if not stream:
            ...
            for i, choice in enumerate(response.get("choices")):
                ...
                if hasattr(output_message, "role") and output_message.role is not None:
                    metadata["role"] = output_message.role
...
def build_output_data(
    ...
    if message is not None:
        ...
        elif message.get("function_call", None) is not None:
            function_call = message.get("function_call")
            tool_calls = [ToolCallData(
                id = "function_call_data",
                function=FunctionCallData(
