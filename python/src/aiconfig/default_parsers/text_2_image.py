...
def construct_regular_output(
    ...
    base_64_value = pillow_image_to_base64_string(image)
    print("64-encoded-image: \n", pillow_image_to_base64_string(image))
    with open(f"/Users/rossdancraig/Documents/out_{execution_count}.txt", "w+", encoding="utf-8") as f:
        f.write(base_64_value)
...
If that doesn't work, you can also try less computationally intensive models. 
        """
        print(disclaimer_print_message)
        ...
        if hasattr(response, 'nsfw_content_detected'): 
            # StableDiffusionPipelineOutput has field nsfw_content_detected but 
            # StableDiffusionXLPipelineOutput does not
            has_nsfw_responses = response.nsfw_content_detected
        
        outputs : List[Output] = []
        for count, (image, has_nsfw) in enumerate(
            itertools.zip_longest(
                response.images or [], # TODO (rossdanlm): check for different non-list types
                has_nsfw_responses,
            )
        ):
            print(f"{count=}")
