...
class HuggingFaceTextGenerationTransformer(ParameterizedModelParser):
    ...
    async def run_inference(
        ...
        model : str = aiconfig.get_model_name(prompt)
        if isinstance(model, str) and model not in self.generators:
            self.generators[model] = pipeline('text-generation', model=model)
