...
# Decide how to the config gets run
run_options = {
    "run_all": True,
    "run last": False
}

# Decide params for a run id
params = {
    "1": {
        'descriptor': "bouncy"
    }
...
# Decide how many times to run the config.
n = 1
    ...
    for nth_execution in range(n):
        ...
        for run_id, run_params in params.items():
            if run_options["run_all"]:
                ...
                # 1. Store outputs in some dictionary shape (ie pandas dataframe, regular dict, etc)
