...
class AIConfig(BaseModel):
    ...
    def _update_model_name(self, name: str, settings: Union[InferenceSettings, None], prompt_name: Optional[str] = None):
        ...
        if prompt_name is not None:
            ...
            elif prompt.metadata.model is None: 
                prompt.metadata.model = ModelMetadata(name=name, settings={})
            elif isinstance(prompt.metadata.model, str):
            ...
            warning_message = "No prompt name was given to update the \
                model name to '{name}'. We are assuming this is intentional \
                and are therefore updating the AIConfig-level settings. If \
                this is a mistake, please rerun the `update_model` function \
                with a specified `prompt_name` argument"
    ...
    def _update_model_settings_for_prompt(self, settings: InferenceSettings, prompt_name: str):
        ...
        metadata_error_message = "Cannot update model settings for prompt \
            '{prompt_name}' because it does not have a model name set for it. \
            Please be sure that a model is set for this prompt. You can do \
            this be calling `update_model()` and passing a model name as an \
            argument."
