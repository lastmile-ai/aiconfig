...
def _run_servers(edit_config: EditConfig) -> Result[None, str]:
    ...
    return result.do(
        _sigint(frontend_procs_ok)
        for frontend_procs_ok in frontend_procs
        for _ in run_backend_server(edit_config)
    )
...
def _run_frontend_server_background() -> Result[list[subprocess.Popen[bytes]], str]:
    ...
    try:
        ...
        p3 = subprocess.Popen(["echo", "start p3"], cwd="src/aiconfig/editor/client")
        return Ok([p1, p2])
