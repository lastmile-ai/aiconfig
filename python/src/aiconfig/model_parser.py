...
from aiconfig.schema import AIConfig, ExecuteResult, Output, Prompt, JSONObject
...
class ModelParser(ABC):
    ...
    async def run_batch(
        ...
        aiConfig: "AIConfigRuntime",
        options: Optional["InferenceOptions"] = None,
        params_list: Optional[list[JSONObject]] = None,
    ) -> None:
        """
        Execute model inference in batch using paramters from params_list

        Args:
            prompt (Prompt): The prompt to be used for inference.
            aiConfig (AIConfig): The AIConfig object containing all prompts and parameters.
            options (InferenceOptions, optional): Options that determine how to run inference for the prompt (ie streaming)
            params_list (list[JSONObject], optional): List of parameters to use for inference
        """
