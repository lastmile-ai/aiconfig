...
export default memo(function PromptContainer({
  ...
  // Whenever the prompt input/output changes, we want to track its height so that we can bound
  // the action bar contents (settings, parameters, etc.) to the height of the prompt input and
  // output container
  useEffect(() => {
    const height = cellInputOutputRef.current?.getBoundingClientRect()?.height;
    if (height) {
      debouncedSetHeight(height);
    }
  }, [prompt.input, prompt.outputs]);
