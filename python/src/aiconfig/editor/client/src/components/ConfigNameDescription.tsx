...
export default memo(function ConfigTitleDescription({
  ...
  // Use local component state to show error for empty name
  // AIConfig state will not set empty name
  ...
  const inputSectionRef = useClickOutside(() => setIsEditing(false));
      ...
      {isEditing ? (
        <>
          <TextInput
            ...
            onChange={setName}
            error={nameInput === "" ? "Name cannot be empty" : null}
          />
          <Textarea
