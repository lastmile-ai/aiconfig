...
def init_server_state(app: Flask, edit_config: EditServerConfig) -> Result[None, str]:
    ...
    if edit_config.aiconfig_path:
        ...
        first_supported_model: str = ModelParserRegistry.parser_ids()[0]
        aiconfig_runtime.add_prompt("prompt_1", Prompt(name="prompt_1", input="", metadata=PromptMetadata(model=first_supported_model)))
