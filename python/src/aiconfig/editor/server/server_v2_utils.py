...
def schedule_operation_task(
    ...
    def _wrap():
        return asyncio.run(_operation_task())

    operation_task = asyncio.create_task(asyncio.to_thread(_wrap))
...
def _handle_new_command_while_operation_task_running(
    ...
    match command:
        case server_common.Cancel():
            logger.warning("Received cancel command while operation task is running. Cancelling operation.")
