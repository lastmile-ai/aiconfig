...
def _run_method_endpoint(method_name: MethodName, signature: dict[str, Type[Any]]) -> FlaskResponse:
    ...
    LOGGER.info("Made operation. Running with request json")
...
def load() -> FlaskResponse:
    ...
    if path is None:
        ...
        path_val = get_validated_path(path)
...
def save() -> FlaskResponse:
    ...
    def _op(aiconfig: AIConfigRuntime, op_args: OpArgs) -> Result[None, str]:
        method_name = MethodName("save")
        return safe_run_aiconfig_method(aiconfig, method_name, op_args)
...
async def run() -> FlaskResponse:
    ...
    op_args = Ok(OpArgs({"prompt_name": prompt_name, "options": inference_options}))
...
def add_prompt() -> FlaskResponse:
    ...
    return _run_method_endpoint(method_name, signature)
