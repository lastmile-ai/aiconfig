...
def run_backend_server(edit_config: EditServerConfig, cli_config_path: str) -> Result[str, str]:
    ...
    res_server_state_init = init_server_state(app, edit_config, cli_config_path)
...
@app.route("/api/get_cli_config", methods=["GET"])
def get_cli_config() -> FlaskResponse:
    state = get_server_state(app)
    ...
    def _yaml_deserialize(yaml_str: str) -> Result[dict[str, Any], str]:
        try:
            ...
            if out is None:
                return Err("YAML string not parsed")
            else:
                return Ok(out)
        except Exception as e:
    ...
    match cli_config_object:
        case Ok(cli_config_object_):
            return FlaskResponse(({"message": "got cli_config", "data": cli_config_object_}, 200))
        case Err(e):
            return FlaskResponse(({"message": f"failed to get cli_config: {e}", "data": None}, 400))


@app.route("/api/set_cli_config", methods=["POST"])
def set_cli_config() -> FlaskResponse:
    state = get_server_state(app)
    request_json = request.get_json()
    try:
        new_contents = request_json.get("new_contents")
        serialized = yaml.dump(new_contents)
        with open(state.cli_config_path, "w") as f:
            f.write(serialized)

        return HttpResponseWithAIConfig(
            message=f"Successfully set cli_config to {serialized}",
            code=200,
            aiconfig=None,
        ).to_flask_format()
    except Exception as e:
        return HttpResponseWithAIConfig(
            message=f"Failed to set cli_config: {e}",
            code=400,
            aiconfig=None,
        ).to_flask_format()
