...
def extract_json_from_chat_completion(chat_completion: openai_types.ChatCompletion) -> Result[test_suite_common.SerializedJSON, str]:
    choice = chat_completion.choices[0]
    message = choice.message
    if message.tool_calls is None:
        return Err("No tool calls found")

    return Ok(test_suite_common.SerializedJSON(message.tool_calls[0].function.arguments))
...
def make_fn_completion_text_to_serialized_json(
    ...
    def _chat_completion_create(output_datum: str) -> Result[test_suite_common.SerializedJSON, str]:
    ...
    out: test_suite_common.CompletionTextToSerializedJSON = _chat_completion_create
    return out
