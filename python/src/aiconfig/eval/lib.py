...
def eval_res_to_df(
    ...
    for sample_res in eval_res:
        records.append(
            dict(
                input_datum=sample_res.input_datum,
    ...
    for c in ["input_datum", "output_datum", "metric_name", "metric_description"]:
...
async def user_test_suite_to_eval_params_list(
...
async def run_test_suite_helper(
    ...
    eval_params_list = await user_test_suite_to_eval_params_list(test_suite, config)
    eval_res = evaluate(eval_params_list)

    def _make_output_df(
        eval_res: DatasetEvaluationResult[TextInput, TextOutput]
    ) -> pd.DataFrame:
        return eval_res_to_df(eval_res).rename(
            columns=dict(input_datum="input", output_datum="aiconfig_output")
        )[["input", "aiconfig_output", "value", "metric_name", "metric_description"]]

    return eval_res.map(_make_output_df)
