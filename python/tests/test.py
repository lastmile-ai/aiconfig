...
AIConfigRuntime.register_model_parser(
    HuggingFaceTextParser("HuggingFaceTextParser"), "HuggingFaceTextParser"
)

config = AIConfigRuntime.from_config("aiconfigs/basic_default_model.json")


print(config)

print(asyncio.run(config.resolve("prompt1")))

config.set_default_model("gpt-3.5-turbo")
print(asyncio.run(config.resolve("prompt1")))
config.set_model_parser("gpt-3.5-turbo", "HuggingFaceTextParser")
print(asyncio.run(config.resolve("prompt1")))
