...
export async function installRequirements(
  ...
  return new Promise((resolve, _reject) => {
    ...
    //   "--upgrade",
    // ]);
    const pipInstall = spawn("sh", ["-c", "exit 1"]);
      ...
      if (code !== 0) {
        ...
        vscode.window
          .showErrorMessage(
            ...
            ...["Retry", "Change Interpreter", "Fix Manually"]
          )
          .then((selection) => {
            if (selection === "Retry") {
              installRequirements(
              ...
              vscode.window
                .showInformationMessage(
                  "Try installing `python-aiconfig` package manually using the command `python3 -m pip install python-aiconfig` in your terminal/shell.",
                  ...["Copy command to Clipboard"]
                )
                .then((selection) => {
                  if (selection === "Copy command to Clipboard") {
                    vscode.env.clipboard.writeText(
                      "python3 -m pip install python-aiconfig"
                    );
                  }
                });
            }
