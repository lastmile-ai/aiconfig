...
async function setupEnvironmentVariables(context: vscode.ExtensionContext) {
  ...
  if (fs.existsSync(envPath)) {
    ...
    // TODO: Figure out how to ensure that walkthrough tab doesn't get lost
    // when running this command and opening new doc
    const doc = await vscode.workspace.openTextDocument(envPath);
    if (doc) {
      vscode.window.showTextDocument(doc, {
        preview: false,
        // Tried using vscode.ViewColumn.Beside but that creates a split view
