...
async function createNewAIConfig(
  ...
  const newConfigName = await vscode.window.showInputBox({
    ...
    validateInput: (input) => {
      const nameValidationError = validateConfigName(input, mode);
      if (nameValidationError) {
        return nameValidationError;
      }

      return fs.existsSync(input) ? "File already exists" : null;
    },
  });

  const newConfigFilePath = workspaceUri
    ? path.join(workspaceUri.path, newConfigName)
    : newConfigName;
