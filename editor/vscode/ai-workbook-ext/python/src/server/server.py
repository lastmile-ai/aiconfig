...
from .queue_iterator import STOP_STREAMING_SIGNAL, QueueIterator
...
def run_backend_server(
    ...
    LOGGER.setLevel(start_config.log_level)
    LOGGER.info("Start server config: %s", start_config.model_dump_json())
    LOGGER.info(f"Starting server on http://localhost:{start_config.server_port}")

    app.server_state = ServerState()  # type: ignore
    res_server_state_init = init_server_state(app, start_config, aiconfigrc_path)
    match res_server_state_init:
        case Ok(_):
            LOGGER.info("Initialized server state")
            debug = start_config.server_mode in [
                ServerMode.DEBUG_BACKEND,
                ServerMode.DEBUG_SERVERS,
            ]
            LOGGER.info(f"Running in {start_config.server_mode} mode")
            app.run(port=start_config.server_port, debug=debug, use_reloader=debug)
            return Ok("Done")
        case Err(e):
...
@app.route("/api/load_with_content", methods=["POST"])
def load_with_content() -> FlaskResponse:
    ...
    # TODO: saqadri - clean up this tempfile stuff and just operate in-memory
    with tempfile.NamedTemporaryFile(
        mode="w+t", suffix=f".aiconfig.{mode}"
    ) as aiconfig_temp_file:
        aiconfig_temp_file.write(content)
        aiconfig_temp_file.seek(0)
        aiconfig = AIConfigRuntime.load(aiconfig_temp_file.name)
        state.aiconfig = aiconfig
        return HttpResponseWithAIConfig(
...
def load() -> FlaskResponse:
    ...
    if path is None:
        ...
        if aiconfig is None:
            ...
            return HttpResponseWithAIConfig(
                message="AIConfig already loaded. Here it is!", aiconfig=aiconfig
            ).to_flask_format()
    else:
...
@app.route("/api/save_to_string", methods=["POST"])
def save_to_string() -> FlaskResponse:
    state = get_server_state(app)
    aiconfig = state.aiconfig
    request_json = request.get_json()
    mode: str = request_json.get("mode", "json")
    ...
    if aiconfig is None:
        ...
        # TODO: saqadri - update core SDK to return the serialized aiconfig string
        # Then clean up this tempfile stuff and just operate in-memory
        with tempfile.NamedTemporaryFile(
            mode="w+t", suffix=f".aiconfig.{mode}"
        ) as aiconfig_temp_file:
            aiconfig.save(
                config_filepath=aiconfig_temp_file.name,
                include_outputs=include_outputs,
                mode=mode,
            )
            aiconfig_temp_file.seek(0)
            aiconfig_string = aiconfig_temp_file.read()
...
def save() -> FlaskResponse:
    ...
    match res_path_val:
        ...
        case Err(e):
            return HttpResponseWithAIConfig(
                message=f"Failed to save AIConfig: {e}", code=400, aiconfig=None
            ).to_flask_format()
...
def create() -> FlaskResponse:
    ...
    match aiconfig:
        ...
        case Err(e):
            return HttpResponseWithAIConfig(
                message=f"Failed to create AIConfig: {e}", code=400, aiconfig=None
            ).to_flask_format()
...
def run() -> FlaskResponse:
    ...
    def generate(cancellation_token_id: str):  # type: ignore
        ...
        def create_error_payload(message: str, code: int):
            ...
            return json.dumps(
                {"error": {"message": message, "code": code, "data": aiconfig_json}}
            )
...
def update_model() -> FlaskResponse:
    ...
    operation_args: Result[OpArgs, str] = result.Ok(
        OpArgs(
            {"model_name": model_name, "settings": settings, "prompt_name": prompt_name}
        )
    )
