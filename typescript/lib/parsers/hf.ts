...
export class HuggingFaceTextGenerationModelParser extends ParameterizedModelParser<TextGenerationArgs> {
  ...
  public serialize(
    ...
    modelMetadata = aiConfig.generateModelMetadata(
  ...
  public async run(
    ...
    if (stream) {
      ...
      const output = await construct_stream_output(
        response,
      ...
      const output = constructRegularOutput(response);
      return output;
    }
  }
 ...
 * @returns
 */
async function construct_stream_output(
  ...
  (async () => {
    for await (const iteration of response) {
      ...
      options.callbacks!.streamCallback(delta, accumulated_message, 0);
...
function constructRegularOutput(response: TextGenerationOutput): Output {
  const metadata = {};
  const data = response;
