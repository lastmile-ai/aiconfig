...
class GeminiModelParser(ParameterizedModelParser):
    ...
    async def deserialize(self, prompt: Prompt, aiconfig: "AIConfigRuntime", params: Optional[Dict] = None) -> Dict:
        ...
        if contains_prompt_template(prompt):
            ...
            completion_data['contents'] = parameterize_support_gemini_input_data(completion_data['contents'], prompt, aiconfig, params)
...
def parameterize_support_gemini_input_data(part: Any, prompt: Prompt, aiconfig: "AIConfigRuntime", input_params):
    """
    Gemini API supports a variety of inputs, some of which are called 'parts', coined by Gemini API.
    This function parameterizes the input based on the type of input.
    *Important: This supports a subset of the Gemini API inputs. Specifically, types that contain strings (and not protos, images, etc)
    ...
    if isinstance(part, str):
        ...
        return [parameterize_support_gemini_input_data(item, prompt, aiconfig, input_params) for item in part]
    elif isinstance(part, dict):
        # Expect "parts" key to be present in role dict
        part = copy.deepcopy(part)
        part["parts"] = parameterize_support_gemini_input_data(part["parts"], prompt, aiconfig, input_params)
        return part
