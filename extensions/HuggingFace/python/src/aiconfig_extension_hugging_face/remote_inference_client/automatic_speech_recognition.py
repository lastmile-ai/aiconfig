...
class HuggingFaceAutomaticSpeechRecognitionRemoteInference(ModelParser):
    ...
    async def deserialize(
        ...
        if len(inputs) > 1:
            raise ValueError(
                f"Multiple audio inputs are not supported for the HF Automatic Speech Recognition api. Please specify a single audio input attachment."
            )

        completion_data["audio"] = inputs[0]
