...
```
```python
callback_manager = new CallbackManager([my_logging_callback]);
aiConfigRuntimeInstance.set_callback_manager(callbackManager);
```
...
callback_manager = CallbackManager(callbacks, custom_timeout)
```
#### Error Handling
